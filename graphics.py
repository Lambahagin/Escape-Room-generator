...
def render_game_scene(state_mode, progress, total_time, elapsed_time=0):
    """
    Tegner spillets grafik. Version 5.5 - Zero-point koordinater.
    Retter fejlen hvor monsteret startede for langt fremme.
    """
    ...
    # X-Koordinater
    start_platform_x = 50   # Hvor monsteret starter
    player_start_x = 150    # Hvor spilleren starter (trin 0)
    step_size = 100         # Afstand mellem trin
    
    # Beregn spillerens aktuelle mål-position
    # Trin 0 = 150, Trin 1 = 250, osv.
    target_player_x = player_start_x + (progress * step_size)
    
    # --- 2. BEREGN MONSTER POSITION (JAGT LOGIK) ---
    
    monster_current_x = start_platform_x
    monster_anim = ""
    
    if state_mode == 'PLAYING' and total_time > 0:
        # Hvor stor procentdel af tiden er gået?
        percent_done = min(elapsed_time / total_time, 1.0)
        
        # Total distance fra start-platform til spillerens NUÆRENDE position
        total_hunt_distance = target_player_x - start_platform_x
        
        # Monsterets start-position i denne frame
        ...
        # ANIMATION: 
        # Vi animerer 'transform' attributten fra nuværende X til spillerens X.
        # VIGTIGT: Vi beholder Y-koordinaten konstant i animationen.
        ...
        # Ved død står monsteret lige oven i spilleren (eller hvor spilleren var)
    ...
    # --- 3. BEREGN SPILLER ANIMATION (DØD) ---
    player_y_anim = ""
    player_opacity = "1.0"
    
    if state_mode == 'DEATH':
        # Spilleren falder ned (Y-aksen øges)
        # from="0 0" betyder "fra nuværende position relative til gruppen"
        player_y_anim = '<animateTransform attributeName="transform" type="translate" from="0 0" to="0 300" dur="1.5s" fill="freeze" />'
        player_opacity = "0.6" # Fader lidt ud
    ...
    # --- FIGURER (ZERO-POINT DRAWING) ---
    # Vi tegner figurerne omkring (0,0) og flytter dem med <g transform="...">
    # Dette sikrer at positionen altid er korrekt.
    ...
    # 6. SKYGGE MONSTERET
    # Hvis vi spiller, bruger vi animationen i translate.
    # Hvis vi er døde/briefing, bruger vi den faste position.
    
    transform_value = f'translate({monster_current_x}, {monster_y})'
    # Bemærk: monster_anim overskriver transform attributten hvis den er sat
    
    html += f'<g transform="{transform_value}">'
    html += monster_anim
    
    # Monster tegning (centreret om 0,0)
    # Krop
